...
describe('rotation engine', () => {
  ...
  it('changes only identities when applying a preset team', () => {
    ...
    const next = applyTeamIdentities(defaultMembers, rotationPresets[0].team);
    const after = calculateTeam(next, duration);
    expect(next.map((member) => member.name)).toEqual(rotationPresets[0].team);
  ...
  it('repairs damaged explorer and progress state', () => {
    const team = normalizeExplorerTeam(['Синку', 'Shinku', 'Хатор', 'missing'], rotationPresets[0].team);
