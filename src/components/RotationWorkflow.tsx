...
import { RotationStepTerms } from './RotationStepTerms';
    ...
    {view === 'plan' ? <div className="rotation-plan">
      ...
      {(['setup', 'burst', 'recovery'] as RotationPhase[]).map((phase) => {
        ...
        return <section className={`rotation-plan-phase phase-${phase}`} key={phase}><header><span>{phaseLabels[phase][locale]}</span><small>{phaseSteps.length} {ru ? 'шаг.' : 'steps'}</small></header>{phaseSteps.map((step) => {
          ...
          return <label className={`rotation-plan-row ${isComplete ? 'complete' : ''}`} key={step.id}><input type="checkbox" checked={isComplete} onChange={() => onToggleStep(step.id)} /><span className="rotation-plan-number">{isComplete ? <Check size={15} /> : globalIndex + 1}</span><span className="rotation-plan-action"><b>{localizedCharacterName(step.actor, locale)}</b><em>{actionLabels[step.action][locale]}{step.optional ? ` · ${ru ? 'необязательно' : 'optional'}` : ''}</em><p>{step.instruction[locale]}</p><RotationStepTerms presetId={preset.id} step={step} compact /></span><span className="rotation-plan-purpose">{purposeForStep(step, locale)}</span><span className="rotation-plan-outcome"><Sparkles size={14} />{step.outcome[locale]}</span></label>;
        })}</section>;
      })}
      ...
      <RotationStepTerms presetId={preset.id} step={currentStep} />
