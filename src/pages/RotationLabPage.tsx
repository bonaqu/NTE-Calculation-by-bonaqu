...
import type { RotationPhase } from '../types';
...
function isStoredTeam(value: unknown): value is TeamMemberInput[] {
  return Array.isArray(value)
    && value.length >= 4
    && value.slice(0, 4).every((entry) => typeof entry === 'object' && entry !== null && typeof (entry as { name?: unknown }).name === 'string');
}

export function RotationLabPage() {
  const { locale } = useI18n();
  const ru = locale === 'ru';
  const [presetId, setPresetId] = useLocalStorage<string>('nte.rotation.preset.v1', rotationPresets[0].id, {
  ...
  const [explorerTeam, setExplorerTeam] = useLocalStorage<string[]>('nte.rotation.explorer.v1', rotationPresets[0].team, {
    normalize: (value) => normalizeExplorerTeam(value, rotationPresets[0].team),
  });
  const [transferError, setTransferError] = useState(false);

  const preset = rotationPresetById.get(presetId) ?? rotationPresets[0];
  ...
  return <div className="page calc-page rotation-page">
    <header className="page-heading"><div><span>{ru ? 'ЛАБОРАТОРИЯ РОТАЦИЙ' : 'ROTATION LAB'}</span><h1>{ru ? 'Ротации и Esper Cycle без догадок' : 'Sourced rotations and Esper Cycles'}</h1><p>{ru ? 'Тренируй подтверждённый порядок действий, проверяй доступные реакции команды и переносите состав в калькулятор. Точные секунды не выдумываются там, где источник их не публикует.' : 'Practice sourced action order, inspect the reactions your team can form, and transfer the composition into the calculator. Exact seconds are never invented when the source does not publish them.'}</p></div></header>
    ...
    <div className="summary-strip rotation-summary"><Metric label={ru ? 'Персонажей' : 'Characters'} value={preset.team.length} /><Metric label={ru ? 'Шагов' : 'Steps'} value={preset.steps.length} /><Metric label={ru ? 'Esper Cycle в плане' : 'Planned Cycles'} value={preset.cyclePlan.length} /><Metric label={ru ? 'Прогресс тренировки' : 'Training progress'} value={`${completedCount}/${preset.steps.length}`} note={`${Math.round(completedCount / preset.steps.length * 100)}%`} /></div>
    ...
    <section className="rotation-workflow" aria-label={ru ? 'Шаги ротации' : 'Rotation steps'}>
      ...
      {phases.map((phase) => {
        ...
        return <section className={`rotation-phase phase-${phase}`} key={phase}><header><span>{phaseTitle[phase]}</span><small>{phaseSteps.length} {ru ? 'шаг.' : 'steps'}</small></header><div>{phaseSteps.map((step) => {
          ...
          return <label className={`rotation-step ${isComplete ? 'complete' : ''}`} key={step.id}><input type="checkbox" checked={isComplete} onChange={() => toggleStep(step.id)} /><span className="step-check" aria-hidden="true">{isComplete ? <Check size={16} /> : stepNumber.get(step.id)}</span><div className="step-copy"><div className="step-title"><b>{localizedCharacterName(step.actor, locale)}</b><span>{step.action}</span>{step.optional ? <em>{ru ? 'необязательно' : 'optional'}</em> : null}{cycle ? <strong>{localizedCycleName(cycle.id, locale)}</strong> : null}</div><p>{step.instruction[locale]}</p><small><Sparkles size={13} /> {step.outcome[locale]}</small></div></label>;
        })}</div></section>;
      })}
    </section>
