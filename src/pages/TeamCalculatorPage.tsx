...
const labels: Record<'ru' | 'en', Record<NumericMemberKey | EnemyKey, string>> = {
  ru: {
    ...
    skillMultiplier: 'Множитель одного одинакового попадания, % ATK', hits: 'Одинаковых попаданий за применение', actionsPerRotation: 'Применений за ротацию', damageBonus: 'Личный бонус урона, %',
...
export function TeamCalculatorPage() {
  ...
  return <div className="page calc-page">
    ...
    <QuickStart title={ru ? 'Быстрый старт' : 'Quick start'} steps={ru ? [
      ...
      'Введи итоговую ATK, общий бонус урона и суммарный множитель одной атаки или серии.',
    ...
    <div className="disclaimer top-note"><Users size={18} /><span>{ru ? 'Выбор персонажа подставляет только имя, роль, атрибут и тип дуги. Сайт не выдумывает множители навыков: введи проверенное значение сам. Быстрый общий множитель объединяет подробные попадания только после твоего изменения поля.' : 'Character selection fills identity metadata only. The site does not invent skill multipliers: enter a verified value yourself. The quick total multiplier flattens detailed hits only after you edit it.'}</span></div>
    ...
    <div className="team-layout"><div className="member-stack">{normalizedMembers.map((member, index) => {
      ...
      return <Panel key={`${member.id}-${index}`} className="member-panel simplified-member-panel">
        ...
        <div className="member-core-grid">
          ...
          <Field label={ru ? 'Суммарный множитель за применение, % ATK' : 'Total multiplier per use, % ATK'} hint={ru ? 'Сложи разные части серии: 120% + 180% = 300%.' : 'Add different parts of a sequence: 120% + 180% = 300%.'} type="number" min="0" value={Math.round(multiplierPerUse * 100) / 100} onChange={(event) => updateTotalMultiplier(index, Number(event.target.value))} />
        ...
        <div className="sequence-formula" aria-live="polite"><span>{ru ? 'Как это считается' : 'Calculation interpretation'}</span><strong>{member.hits === 1
          ? `${multiplierText(multiplierPerUse)}% × ${multiplierText(uses)} = ${multiplierText(rotationMultiplier)}% ATK ${ru ? 'за ротацию' : 'per rotation'}`
          : `${multiplierText(member.skillMultiplier)}% × ${multiplierText(member.hits)} ${ru ? 'попад. ×' : 'hits ×'} ${multiplierText(uses)} = ${multiplierText(rotationMultiplier)}% ATK ${ru ? 'за ротацию' : 'per rotation'}`}</strong></div>

        <details className="member-advanced-fields"><summary><SlidersHorizontal size={17} /><span><b>{ru ? 'Подробный ввод' : 'Detailed input'}</b><small>{ru ? 'Разбивка одинаковых попаданий, ATK и командных бонусов' : 'Identical-hit, ATK and team-bonus breakdown'}</small></span><ChevronDown size={18} /></summary><div className="field-grid compact">{advancedMemberFields.map((key) => <Field key={key} label={labels[locale][key]} type="number" value={member[key]} onChange={(event) => updateMember(index, key, Number(event.target.value))} />)}</div><p>{ru ? 'Разные по силе попадания проще сразу сложить в суммарный множитель. Подробная пара нужна только для одинаковых попаданий: например, 150% × 4. Изменение суммарного множителя переносит результат в одно виртуальное попадание; изменение итоговой ATK или общего бонуса аналогично объединяет их подробные компоненты.' : 'Different hit values are easiest to add into the total multiplier. The detailed pair is only needed for identical hits, such as 150% × 4. Editing the total multiplier stores it as one virtual hit; editing Effective ATK or Combined DMG Bonus similarly merges their detailed components.'}</p></details>
      </Panel>;
    })}</div>
      ...
      <aside><Panel className="sticky-results"><div className="panel-title"><Gauge size={20} /><div><h2>{ru ? 'Кто сколько урона вносит' : 'Rotation contribution'}</h2><p>{ru ? 'Урон уже учитывает среднее значение критических попаданий.' : 'Expected damage with averaged crit.'}</p></div></div><div className="contribution-list">{result.members.map((member) => <div key={member.id}><span><b>{localizedCharacterName(member.name, locale)}</b><small>{formatNumber(member.rotationDamage)}</small></span><strong>{(member.share * 100).toFixed(1)}%</strong><i><u style={{ width: `${member.share * 100}%` }} /></i></div>)}</div><Field label={ru ? 'Длительность всей ротации, сек' : 'Rotation duration, sec'} type="number" min="1" value={duration} onChange={(event) => setDuration(Number(event.target.value))} /><h3 className="subsection-label">{ru ? 'Характеристики врага' : 'Enemy profile'}</h3><div className="enemy-grid">{enemyFields.map((key) => <Field key={key} label={labels[locale][key]} type="number" value={enemy[key]} onChange={(event) => updateEnemy(key, Number(event.target.value))} />)}</div><FieldHelp title={ru ? 'Что означают основные поля?' : 'What do the main fields mean?'} terms={ru ? [
        { term: 'Итоговая ATK', description: 'Готовое значение атаки после дуги, процентов и прибавок числом. Его можно взять из экрана характеристик.' },
        { term: 'Общий бонус урона', description: 'Сумма личного бонуса урона и бонусов, которые этому персонажу даёт команда.' },
        { term: 'Суммарный множитель за применение', description: 'Общий процент ATK одной выбранной атаки, навыка или серии. Разные части серии складываются один раз.' },
      ...
      ]} /><div className="model-note"><Users size={18} />{ru ? 'Пассивные эффекты, реакции и точный порядок действий пока задаются общими бонусами, поэтому результат остаётся оценкой.' : 'Passives, reactions and exact action order are represented through explicit aggregate bonuses, so the result remains an estimate.'}</div></Panel></aside>
    </div>
  </div>;
}
