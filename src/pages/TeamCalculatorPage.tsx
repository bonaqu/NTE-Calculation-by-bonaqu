...
import { AttackSequenceBuilder } from '../components/AttackSequenceBuilder';
...
export function TeamCalculatorPage() {
  ...
  const [sequenceStore, setSequenceStore] = useLocalStorage<AttackSequenceStore>(
    'nte.team.sequence-builder.v1',
    emptyAttackSequenceStore(),
    { normalize: normalizeAttackSequenceStore },
  );
  ...
  const updateSequenceDraft = (characterName: string, draft: AttackSequenceDraft) => setSequenceStore((current) => updateAttackSequenceDraft(current, characterName, draft));
  const applySequenceDraft = (index: number, draft: AttackSequenceDraft) => setMembers((current) => current.map((item, itemIndex) => itemIndex === index
    ? applyAttackSequence(item, draft).member
    : item));
  ...
  return <div className="page calc-page">
    <header className="page-heading"><div><span>{ru ? 'РАСЧЁТ КОМАНДЫ' : 'TEAM CALCULATION'}</span><h1>{ru ? 'Калькулятор урона команды' : 'Team rotation calculator'}</h1><p>{ru ? 'Выбери четырёх персонажей и задай для каждого одну основную атаку, навык или серию. Разные части серии теперь можно сложить прямо внутри сайта.' : 'Choose four characters and describe one main attack, skill or sequence for each. Different sequence parts can now be added directly inside the site.'}</p></div><button className="button ghost" onClick={reset}><RotateCcw size={17} /> {ru ? 'Вернуть пример' : 'Restore sample'}</button></header>
    ...
    <QuickStart title={ru ? 'Быстрый старт' : 'Quick start'} steps={ru ? [
      ...
      'Введи итоговую АТК и общий бонус урона. Для сложной серии открой конструктор и добавь её части отдельными строками.',
      'Перенеси готовую сумму в расчёт, укажи длительность ротации, затем сверь и подтверди текущие значения.',
    ] : [
      'Select “Start with my own data” to clear demo combat values while keeping the selected characters.',
      'Enter effective ATK and combined damage bonus. Open the builder for a complex sequence and add its parts as separate rows.',
      'Apply the completed sum, set rotation duration, then review and confirm the current inputs.',
    ]} />

    <div className="disclaimer top-note"><Users size={18} /><span>{ru ? 'Выбор персонажа подставляет только имя, роль, тип эспера и тип дуги. Сайт не выдумывает множители навыков: числа и названия частей серии вводишь ты. Подтверждённые игровые названия доступны в Ротациях и Методологии, но не подставляются как числовые пресеты.' : 'Character selection fills identity metadata only. The site does not invent skill multipliers: sequence values and row names are player-entered. Confirmed game labels are available in Rotations and Methodology but are not inserted as numeric presets.'}</span></div>
    ...
    <div className="team-layout"><div className="member-stack">{normalizedMembers.map((member, index) => {
      ...
      const sequenceDraft = sequenceStore.drafts[member.name] ?? createAttackSequenceDraft(
        member,
        ru ? 'Текущая атака или серия' : 'Current attack or sequence',
      );
      return <Panel key={`${member.id}-${index}`} className="member-panel simplified-member-panel">
        ...
        <div className="member-core-grid">
          ...
          <Field label={ru ? 'Суммарный множитель за применение, % АТК' : 'Total multiplier per use, % ATK'} hint={ru ? 'Для простой атаки введи число сразу; сложную серию разложи ниже.' : 'Enter a simple attack directly or break a complex sequence down below.'} type="number" min="0" value={Math.round(multiplierPerUse * 100) / 100} onChange={(event) => updateTotalMultiplier(index, Number(event.target.value))} />
        ...
        <AttackSequenceBuilder
          ...
          draft={sequenceDraft}
          currentMultiplierPerUse={multiplierPerUse}
          currentUsesPerRotation={uses}
          onChange={(draft) => updateSequenceDraft(member.name, draft)}
          onApply={(draft) => applySequenceDraft(index, draft)}
          onResetFromCurrent={() => updateSequenceDraft(member.name, createAttackSequenceDraft(
            member,
            ru ? 'Текущая атака или серия' : 'Current attack or sequence',
          ))}
        />

        <details className="member-advanced-fields"><summary><SlidersHorizontal size={17} /><span><b>{ru ? 'Подробный ввод' : 'Detailed input'}</b><small>{ru ? 'Разбивка одинаковых попаданий, АТК и командных бонусов' : 'Identical-hit, ATK and team-bonus breakdown'}</small></span><ChevronDown size={18} /></summary><div className="field-grid compact">{advancedMemberFields.map((key) => <Field key={key} label={labels[locale][key]} type="number" value={member[key]} onChange={(event) => updateMember(index, key, Number(event.target.value))} />)}</div><p>{ru ? 'Разные по силе попадания разложи в конструкторе серии. Подробная пара нужна только для одинаковых попаданий: например, 150% × 4. Перенос из конструктора записывает готовую сумму как одно виртуальное попадание и не меняет остальные параметры.' : 'Use the sequence builder for different hit values. The detailed pair is only needed for identical hits, such as 150% × 4. Applying a built sequence stores the completed sum as one virtual hit without changing other inputs.'}</p></details>
      </Panel>;
    })}</div>
      ...
      <aside><Panel className="sticky-results"><div className="panel-title"><Gauge size={20} /><div><h2>{ru ? 'Вклад персонажей в урон' : 'Rotation contribution'}</h2><p>{ru ? 'Результат уже учитывает среднее значение критических попаданий.' : 'Expected damage with averaged crit.'}</p></div></div><div className="contribution-list">{result.members.map((member) => <div key={member.id}><span><b>{localizedCharacterName(member.name, locale)}</b><small>{formatNumber(member.rotationDamage)}</small></span><strong>{(member.share * 100).toFixed(1)}%</strong><i><u style={{ width: `${member.share * 100}%` }} /></i></div>)}</div><Field label={ru ? 'Длительность всей ротации, сек' : 'Rotation duration, sec'} type="number" min="1" value={duration} onChange={(event) => setDuration(Number(event.target.value))} /><h3 className="subsection-label">{ru ? 'Характеристики врага' : 'Enemy profile'}</h3><div className="enemy-grid">{enemyFields.map((key) => <Field key={key} label={labels[locale][key]} type="number" value={enemy[key]} onChange={(event) => updateEnemy(key, Number(event.target.value))} />)}</div><FieldHelp title={ru ? 'Что означают основные поля?' : 'What do the main fields mean?'} terms={ru ? [
        ...
        { term: 'Суммарный множитель за применение', description: 'Общий процент АТК одной выбранной атаки, навыка или серии. В конструкторе разные части сначала складываются.' },
        { term: 'Применений за ротацию', description: 'Сколько раз готовая атака или вся серия происходит за одну полную ротацию команды.' },
        ...
        { term: 'Total multiplier per use', description: 'The total % ATK of one selected attack, skill or sequence. Builder rows are added first.' },
        { term: 'Uses per rotation', description: 'How many times the completed attack or whole sequence occurs during one full team rotation.' },
