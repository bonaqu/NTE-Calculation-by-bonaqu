...
import { arcDirectory, type ArcDirectoryRarity, type ArcDirectoryType } from '../arc-directory';
...
export function DatabasePage() {
  ...
  const arcs = useMemo(() => arcDirectory.filter((row) => {
  ...
  return <div className="page calc-page database-page">
    ...
    <div className="database-toolbar">
      <div className="segmented" role="tablist" aria-label={ru ? 'Раздел базы данных' : 'Database section'}>
        ...
        <button role="tab" aria-selected={tab === 'arcs'} className={tab === 'arcs' ? 'active' : ''} onClick={() => setTab('arcs')}>{ru ? `Дуги · ${arcDirectory.length}` : `Arcs · ${arcDirectory.length}`}</button>
      </div>
    ...
    {tab === 'arcs' ? <>
      {arcs.length > 0 ? <section className="arc-directory-grid" aria-live="polite">{arcs.map((arc) => <details className={`arc-directory-card rarity-${arc.rarity.toLowerCase()}`} key={arc.id}>
        <summary>
        ...
        <div className="arc-directory-details"><div className="arc-effect-title"><BookOpenText size={17} /><b>{ru ? 'Эффект M1 → M5' : 'M1 → M5 effect'}</b></div><p>{arc.effect[locale]}</p><div className="arc-source-line"><span><CheckCircle2 size={14} /> Prydwen Institute</span><small>{ru ? 'Проверено' : 'Verified'}: {arc.verifiedAt}</small><a href="https://www.prydwen.gg/neverness-to-everness/arcs" target="_blank" rel="noreferrer">{ru ? 'Источник' : 'Source'}</a></div></div>
      </details>)}</section> : <Panel className="empty-database"><Search size={24} /><h2>{ru ? 'Дуги не найдены' : 'No Arcs found'}</h2><p>{ru ? 'Измени запрос или сбрось фильтры.' : 'Change the query or reset the filters.'}</p><button className="button ghost" onClick={() => { setQuery(''); resetArcFilters(); }}>{ru ? 'Сбросить' : 'Reset'}</button></Panel>}
    </> : null}
  </div>;
}
