import { ExternalLink, FlaskConical, GitBranch, ShieldAlert } from 'lucide-react';
...
export function MethodologyPage() {
  ...
  return <div className="page calc-page">
    ...
    <Panel className="benchmark-method-panel"><div className="panel-title"><GitBranch size={20} /><div><h2>{ru ? 'Почему бенчмарки разделены' : 'Why benchmarks are separated'}</h2><p>{ru ? 'Результаты из разных таблиц не смешиваются в один рейтинг.' : 'Results from different tables are not mixed into one ranking.'}</p></div></div><p>{ru ? 'Публичная таблица Prydwen и расширенный скриншот Rivyn используют разные наборы Mixing Level и разную детализацию. Каждый сценарий отображается отдельно со своим источником, условиями и датой проверки.' : 'The public Prydwen table and the extended Rivyn screenshot use different Mixing Level sets and expose different detail. Each scenario is displayed separately with its own source, conditions and verification date.'}</p><div className="benchmark-method-list">{arcBenchmarkScenarios.map((scenario) => <div key={scenario.id}><b>{scenario.title[locale]}</b><span>{sources.find((source) => source.id === scenario.sourceId)?.publisher} · {scenario.verifiedAt}</span><p>{scenario.description[locale]}</p></div>)}</div></Panel>
