...
export function ArcExperiencePage() {
  ...
  return <div className="arc-experience-page">
    <div className="page calc-page arc-experience-intro">
      ...
      <CharacterArcGuide />
      <section className="iroi-calculation-intro" aria-labelledby="iroi-calculation-title"><div><span>{ru ? 'ОТДЕЛЬНЫЙ ИНСТРУМЕНТ' : 'SEPARATE TOOL'}</span><h2 id="iroi-calculation-title">{ru ? 'Сравнения и пользовательский расчёт Ирой' : 'Iroi benchmarks and custom calculation'}</h2><p>{ru ? 'Следующий блок относится только к Ирой. Готовые таблицы сохраняют условия своих источников, а режим «Мои характеристики» использует прозрачную частичную модель сайта.' : 'The following block applies only to Iroi. Sourced tables retain their original assumptions, while Custom stats uses the site’s transparent partial model.'}</p></div></section>
    </div>
    <ArcCalculatorPage />
  </div>;
}
